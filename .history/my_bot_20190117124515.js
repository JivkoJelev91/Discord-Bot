...
async function levelingSystem(receivedMessage, userMsg, counter = 0){
    // var result = userMsg % (2 * level) + 1;

    level++;
    divider = divider * level;
     if(userMsg % divider == 0){
         counter++
        await receivedMessage.channel.send('Ти си **' + (counter) + '** ниво! Стига спами вече!');
     }
}
