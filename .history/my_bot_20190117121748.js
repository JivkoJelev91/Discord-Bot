...
async function countMessages(receivedMessage, sender,  fs){
    ...
    levelingSystem(receivedMessage, userData[sender.id].messagesSent)
...
async function levelingSystem(){
     if(userData[sender.id].messagesSent % (2 * level) == 0){
        await receivedMessage.channel.send('Ти си **' + (userData[sender.id].messagesSent % 2 + 1) + '** ниво! Стига спами вече!');
    }
}
