...
async function countMessages(receivedMessage, sender, level,  fs){
    ...
    levelingSystem(receivedMessage, userData, level, sender);

    return userData;
}

async function levelingSystem(receivedMessage, userData, sender){
    console.log(userData[sender.id])
    
    if(userData[sender.id].messagesSent % 10 === 0){
