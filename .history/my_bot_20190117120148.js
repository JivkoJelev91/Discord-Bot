...
async function countMessages(receivedMessage, sender, level,  fs){
    ...
    await levelingSystem(receivedMessage, userData, sender, level);

    return userData;
}

 function levelingSystem(receivedMessage, userData, sender, level){
    console.log(userData[sender.id])
    
    if(userData[sender.id].messagesSent % 2 == 0){
        level++;
        console.log(level)
        receivedMessage.channel.send('Ти си **' + level + '** ниво! Стига спами вече!');
    }
